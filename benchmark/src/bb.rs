...
//!
//! `u32` is the only unbounded integer primitive used by the
//! workloads. The wrapper stands in for a hypothetical default-on
...
//! are intentionally not wrapped, so the wrapper measures what an
