...
fn feedback_guided_tmr_error_pins_hint_stats_and_features() {
    ...
    assert!(
        debug.contains(&format!(
            "stats: {{ accepted: 0, rejected: {}, total_samples: 0, discovered_features: 1, max_corpus_size: 1, required_event_hits: 0 }},",
...
// === Required event coverage (Runner::require_event) ===

#[test]
fn require_event_fails_when_never_reported_in_plain_run() {
    // A run that completes its case budget without a single report of
    // the declared event must fail with RequiredEventNotReached: the
    // vacuous pass (no reach, no evidence) is exactly what the gate
    // exists to prevent.
    let mut runner = noprop::Runner::new(1);
    runner.require_event("must-reach");
    let err = runner
        .run(8, |_ctx| Ok(()))
        .expect_err("a run without the required event must fail");
    assert_eq!(err.kind(), noprop::RunErrorKind::RequiredEventNotReached,);
    assert_eq!(err.required_event_label(), Some("must-reach"));
    assert_eq!(err.case_index(), 8);
    assert_eq!(err.generated().len(), 0, "no failing case produced a trace");
    assert_eq!(err.stats().required_event_hits, 0);
}

#[test]
fn require_event_passes_when_reported_in_plain_run() -> noprop::TestResult {
    let mut runner = noprop::Runner::new(1);
    runner.require_event("must-reach");
    runner.run(8, |ctx| {
        ctx.event("must-reach");
        Ok(())
    })?;
    assert_eq!(runner.stats().required_event_hits, 8);
    Ok(())
}

#[test]
fn require_event_fails_when_never_reported_in_corpus_guided_run() {
    let mut runner = noprop::Runner::new(1);
    runner.require_event("must-reach");
    let err = runner
        .run_feedback_guided(8, |ctx| {
        ...
        .expect_err("a feedback-guided run without the required event must fail");
    assert_eq!(err.kind(), noprop::RunErrorKind::RequiredEventNotReached,);
    assert_eq!(err.required_event_label(), Some("must-reach"));
}

#[test]
fn require_event_passes_when_reported_in_corpus_guided_run() -> noprop::TestResult {
    let mut runner = noprop::Runner::new(1);
    runner.require_event("must-reach");
    runner.run_feedback_guided(8, |ctx| {
        ctx.event("must-reach");
        Ok(())
    })?;
    assert_eq!(runner.stats().required_event_hits, 8);
...
#[test]
#[should_panic(expected = "require_event")]
fn require_event_with_different_label_panics() {
    // Declaring a different label after an existing one would silently
    // drop the first requirement — the vacuous pass the gate exists to
    // prevent — so it must be caught loudly instead.
    let mut runner = noprop::Runner::new(1);
    runner.require_event("first");
    runner.require_event("second");
}

#[test]
fn require_event_with_same_label_is_idempotent() -> noprop::TestResult {
    let mut runner = noprop::Runner::new(1);
    runner.require_event("same");
    runner.require_event("same");
    runner.run(4, |ctx| {
        ctx.event("same");
        Ok(())
...
#[test]
fn require_event_declaration_persists_across_runs_but_hits_reset() {
    // The declaration stays on the runner between runs; only the hit
    // counter resets per run, so the second run must fail again.
    let mut runner = noprop::Runner::new(1);
    runner.require_event("must-reach");
    runner
        .run(4, |ctx| {
    ...
    assert_eq!(runner.stats().required_event_hits, 4);

    let err = runner
        .run(4, |_ctx| Ok(()))
        .expect_err("second run without the event must fail");
    assert_eq!(err.kind(), noprop::RunErrorKind::RequiredEventNotReached,);
}

#[test]
fn require_event_counts_reports_from_rejected_cases() -> noprop::TestResult {
    // Rejected attempts still execute the closure, so their reports of
    // the required event count toward the run-wide total.
    use std::cell::Cell;
    let attempts = Cell::new(0usize);
    let mut runner = noprop::Runner::new(1);
    runner.require_event("must-reach");
    ...
    // 2 rejected + 4 accepted attempts, each reporting once.
    assert_eq!(runner.stats().required_event_hits, 6);
    Ok(())
}

#[test]
fn require_event_count_ignores_the_feature_cap() -> noprop::TestResult {
    // The required event is counted by a dedicated counter outside the
    // SemanticCoverage machinery: even when distinct labels fill the
    // per-case feature budget first, the required label is still
    // counted, so no cap-derived false failure can occur.
    let mut runner = noprop::Runner::new(1);
    runner.require_event("must-reach");
    runner.run_feedback_guided(4, |ctx| {
        for i in 0..64 {
            ctx.event(if i % 2 == 0 { "even" } else { "odd" });
        }
        ctx.event("must-reach");
        Ok(())
    })?;
...
#[test]
fn property_failure_takes_priority_over_required_event() {
    // A property failure is a real bug signal and must win over the
    // coverage gate: the run aborts at the failing case, so the gate is
    ...
    let err = runner
        ...
        .expect_err("the property failure must win");
    assert_eq!(err.kind(), noprop::RunErrorKind::PropertyFailure);
    assert_eq!(err.required_event_label(), None);
}

#[test]
fn required_event_error_report_mentions_the_label() {
    let mut runner = noprop::Runner::new(1);
    runner.require_event("must-reach");
    let err = runner.run(4, |_ctx| Ok(())).expect_err("must fail");
    let display = format!("{err}");
    assert!(
        display.contains("must-reach"),
        "Display must name the unreached label: {display}"
    );
    assert!(
        display.contains("required event"),
        "Display must mention the required event: {display}"
    );
    let debug = format!("{err:?}");
    assert!(
        debug.contains("required_event_not_reached: \"must-reach\""),
        "Debug must name the unreached label: {debug}"
    );
}
